...
uint16_t processSample(uint16_t x) {
    ...
    case RECORDING:
             x0 = xlaudio_adc14_to_q15((((int16_t) buffer[bufindex])<<6)); // Append new sounds to end of buffer
             buffer[bufindex] = xlaudio_adc14_to_q15(0.75*(x+x0))>>6; // Keep upper byte to save storage
